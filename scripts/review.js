...
function buildSummary(data) {
  ...
  orderedKeys.forEach(key => {
    ...
    row.innerHTML = `<span class="r-key">${escapeHtml(formatKey(key))}</span><span class="r-val">${escapeHtml(val)}</span>`;
    container.appendChild(row);
  });

  if (container.children.length === 0) {
    container.innerHTML = `<p style="color:#6c757d;">No submission data found.</p>`;
  }
}
